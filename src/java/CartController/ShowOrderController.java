...
public class ShowOrderController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (role == 1) {
            // Admin sees all
            list = orderDao.findOrdersEntities();
        } else if (role == 2) {
            // Manager sees User (0) and Manager (2)
            list = orderDao.findOrdersByRoles(java.util.Arrays.asList(0, 2));
        } else {
            // User sees only own
            list = orderDao.findByAccount(acc.getAccount());
        }

        request.setAttribute("orders", list);
        request.getRequestDispatcher("privates/showOrder.jsp").forward(request, response);
    }
