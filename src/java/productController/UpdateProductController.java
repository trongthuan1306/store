...
public class UpdateProductController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("privates/sualoai_pro.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ProductsJpaController proDao = new ProductsJpaController();
            Products pro = proDao.findProducts(id);
            pro.setProductName(name);
            pro.setBrief(brief);
            pro.setPostedDate(postedDate);
            pro.setTypeId(y);
            pro.setAccount(x);
            pro.setUnit(unit);
            pro.setPrice(price);
            ...
            if (filePart != null && filePart.getSize() > 0) {
                ...
                if (sourcePath != null) {
                    ...
                    java.nio.file.Files.copy(Paths.get(finalPath), Paths.get(finalSourcePath),
                            java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                    System.out.println("Successfully copied to source: " + finalSourcePath);
                } else {
                    System.out.println(
                            "Warning: Could not determine source path. Image may be lost after Clean and Build.");
                }
                String imagePath = "/images/sanPham/" + newFileName;
                pro.setProductImage(imagePath);
            }
            proDao.edit(pro);
