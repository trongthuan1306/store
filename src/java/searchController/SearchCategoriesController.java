...
public class SearchCategoriesController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        CategoriesJpaController dao = new CategoriesJpaController();
         List<Categories> list = dao.findCategoriesByName(keyword);
        if(list!=null&& !list.isEmpty()){
        request.setAttribute("ds", list);
        request.getRequestDispatcher("cacloai_cate.jsp")
               .forward(request, response);
        }else{
        request.setAttribute("errorCate","Don't Have Categories!!!");
